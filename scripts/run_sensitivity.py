...
def main() -> None:
    ...
    for orb_window, rr, risk in product([5, 15, 30, 60], [1.0, 1.5, 2.0, 3.0], [0.5, 1.0, 2.0]):
        ...
        signals = strat.run(bars, session_open="14:30", account_equity=float(bt["initial_capital"]))
    ...
    _heatmap(df.groupby(["orb_window", "rr"], as_index=False)["sharpe_ratio"].mean(), "sharpe_ratio", "Sharpe Heatmap", Path("output/plots/sensitivity_sharpe_heatmap.png"))
    _heatmap(df.groupby(["orb_window", "rr"], as_index=False)["total_return"].mean(), "total_return", "Total Return Heatmap", Path("output/plots/sensitivity_total_return_heatmap.png"))
    _heatmap(df.groupby(["orb_window", "rr"], as_index=False)["max_drawdown"].mean(), "max_drawdown", "Max Drawdown Heatmap", Path("output/plots/sensitivity_max_drawdown_heatmap.png"))

    robust = df[df["expectancy"] > 0]
    robust_region = robust.groupby(["orb_window", "rr"]).size().reset_index(name="count").sort_values("count", ascending=False)
